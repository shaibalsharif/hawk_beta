        ...
        <div className="absolute w-full min-h-screen   bg-black bg-opacity-80 overflow-y-scroll top-0 left-0 ">
            <div className='h-4/5 flex justify-center gap-2 mt-24 items-center'>
                <div className="bg-dark-2 p-8 rounded-lg w-11/12 md:w-9/12 lg:w-7/12 xl:w-5/12">
                    <h2 className="text-xl font-semibold mb-2">{data ? 'Edit Portfolio' : 'Add Portfolio'}</h2>
                ...
                <div className='bg-dark-2 w-[20%] h-[70vh] rounded-md overflow-hidden overflow-y-scroll'>
                    <h2 className="text-xl font-semibold p-4">{'Images'}</h2>
                 
                    {data.content_list.length ? (<div className='p-2 space-y-4 last:mb-8'>
                        {data.content_list.map(item => <img className='border p-2' src={item.type=='g-drive'?getEmbedLink(item.url):item.url}/>)}
                    </div>) : <></>}
