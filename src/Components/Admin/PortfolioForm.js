...
const PortfolioForm = () => {
  ...
  const handleCategoryActions = (actionType, item, event) => {
    ...
    <div className='p-4'>
      <div className=' p-4 rounded-lg'>
        ...
        <div className='w-full '>
          <button className='flex gap-2 bg-dark-2 my-2 px-4 py-1 rounded-sm font-[500] tracking-widest'
            onClick={(e) => handleCategoryActions('add', null, e)}>
          ...
          <div className='space-y-2 sm:space-y-0 gap-2 sm:grid grid-cols-2 md:grid-cols-3 mb-4 '>
            {categoryList.length ? categoryList.map(item =>
              <div className='' onClick={(e) => { handleCategorySelection(item) }}>
                ...
                <div className={`w-full bg-dark-2 px-2 py-2 h-full border relative ${selectedCategory == item.id ? ' border-green-500' : ' blur-[.8px]'}`}>
                  <p>{item.id}</p>
                  ...
                  <div className=' absolute right-0 top-0 flex gap-2 p-2 bg-[#585858] bg-opacity-10 rounded-lg m-2'>
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"
                      onClick={(e) => { handleCategoryActions('edit', item, e) }}
                    ...
                    {/* HIDE */}
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"
                      onClick={(e) => handleCategoryActions('hide', item, e)}
                      className=" hover:bg-dark-1 bg-opacity-20 hover:scale-105 cursor-pointer p-1 rounded-xl w-8 h-8">
                    ...
                    {/* Delete */}
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"
                      onClick={(e) => handleCategoryActions('delete', item, e)}
                      className=" hover:bg-dark-1 bg-opacity-20 hover:scale-105 cursor-pointer p-1 rounded-xl w-8 h-8">
                    ...
                    <div className={`w-full bg-dark-2 px-2 py-2 border relative ${''}`}>
                      ...
                      <div>
                        ...
                        <img className='w-full md:w-1/3'
                          src={getPhotoUrl(item.photo.type, item.photo.url)} />
                      </div>
                      <div className=' absolute right-0 top-0 flex gap-2 p-2 bg-[#585858] bg-opacity-10 rounded-lg m-2'>
