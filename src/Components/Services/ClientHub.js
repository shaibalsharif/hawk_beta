...
const ClientHub = ({ data }) => {
  ...
  return (
    <div className='min-h-screen bg-dark-2 w-full uppercase flex flex-col justify-center items-start px-4 md:px-[20%] gap-20'>
      <div className='h-[20%] text-5xl text-red-600 tracking-wider font-bold py-10'>
        <p className='w-1/2'>{data?.title}</p>
        <p>{data?.details}</p>
      </div>
      <div className='grid grid-cols-2 md:grid-cols-4 w-full text-3xl  text-center p-8 
      ...
      [&>*:nth-child(8)]:border-r-0 [&>*:nth-child(5)]:border-l-0
       py-20'>
        {data?.content?.sort((el_prev, el_next) => el_next.order > el_prev.order).map(el => {
        ...
        {data?.content?.sort((el_prev, el_next) => el_next.order > el_prev.order)?.slice(0,1).map(el => {
          return (<div className=' w-full h-full flex justify-center items-center  border  py-10 px-4'>
            {<img src={getPhotoUrl(el.image?.type, el.image?.url)} />}
          </div>)
        })}
